...
function updateInfo(characterNumber)
{
	...
	//it will be a number between 0 and 4, telling us which position in the array to get the character's information from
	...
	//we will do this using document.createElement
	...
	//get the cInfo div so we can add the charName element to the page
	...
	//const htmlTemplate = `<img src="images/compostHeap.gif" alt="The character's picture">`;

	const htmlTemplate = `<img src="images/${charImage}" alt="The character's picture">`;

	//put the image information into a variable
...
//use a loop to go  throught all five buttons
...
function changeCharacter(){
	...
	//first, make this button "activated" (by adding the class activated)
	this.classList.add("activated");

	//figure out which number this child is, so we can apply the information about the appropriate character

	let allChildren = this.parentNode.children;

	//because allChildren is not a "true" array, but a collection of HTML elements, we have to conver it to a true array first.
	...
	//we'll use that position to get the current character from the array
	updateInfo(pos);
}
